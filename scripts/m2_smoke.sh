...
jq -n --arg scenario "${second_scenario_id}" --arg name "禁止切换" --argjson lock "${dataset_lock}" \
  '{scenario_id: $scenario, name: $name, description: null, expected_lock_version: $lock}' \
...
jq -e '.error.code == "DATASET_SCENARIO_LOCKED"' "${smoke_dir}/scenario-switch.json" >/dev/null
