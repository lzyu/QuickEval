...
所有非安全方法携带 `X-CSRF-Token`。退出、修改密码和超级管理员重置密码后，使目标用户已有 Redis Session 失效。
新建用户的初始密码固定为 `123456`，并在 Session 用户信息中以 `password_change_required`
标记。该标记为真时，除查看 Session、退出和修改密码外，服务端拒绝所有业务接口。

### 4.2 固定角色与用户管理

QuickEval 不提供可配置的角色或权限管理，固定角色如下：

- `super_admin`（超级管理员）：拥有全部能力，独占用户管理。
- `operator`（运营管理员）：拥有全部业务运营能力，包括评测配置、评测集、评测、Badcase、问题标签和审计日志；不能查看或管理用户。
- `member`（成员）：保持既有评测、Badcase 与内容归属权限。
...
- `PATCH` 只修改姓名、邮箱和角色。
- 状态和密码使用显式命令。
- 新建用户不传密码，初始密码固定为 `123456`；首次登录后必须修改密码。
- 超级管理员“重置为初始密码”会恢复 `123456`、撤销该用户已有 Session，并要求其下次登录后修改密码。
- OA 身份没有本地密码时，改密和重置密码返回 `IDENTITY_NOT_LOCAL`。
- 密码、密码哈希和 Session 永远不进入响应、应用日志或审计 JSON。

审计记录会保存操作者用户名快照；当审计资源为用户时，同时保存该用户主体的用户名快照，供历史查询使用。
...
| 查看启用的评测目录与标签 | 是 | 是 | 是 |
| 管理用户与重置用户密码 | 否 | 否 | 是 |
| 管理对象、场景、标签、评测集 | 否 | 是 | 是 |
| 查看审计日志 | 否 | 是 | 是 |
| 创建独立评测 | 是 | 是 | 是 |
| 编辑、完成、重开或作废自己的评测 | 是 | 是 | 是 |
| 维护其他人的评测 | 否 | 是 | 是 |
| 登记和查看 Badcase | 是 | 是 | 是 |
| 为有效 Badcase 添加备注和截图 | 是 | 是 | 是 |
| 分配、调整状态和维护问题标签 | 是 | 是 | 是 |
| 修改 Badcase 原始问题内容 | 仅创建人 | 是 | 是 |
| 无效化或重新激活 Badcase | 仅创建人 | 是 | 是 |
...
配置实体不提供 `DELETE`。成员只读取启用项，运营管理员和超级管理员可筛选停用项。新关联只能选择启用标签，历史关联继续展示停用标签。
...
权限：评测截图由评测创建人、运营管理员或超级管理员维护；所有成员可以为有效 Badcase 补充定位截图；Badcase 截图只能由上传者、Badcase 创建人、运营管理员或超级管理员删除。
